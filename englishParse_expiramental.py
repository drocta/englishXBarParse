...
#potential TODOs:
#Complementizers (C) (eg. 'that' or none) and Complementizer Phrase (CP) ?
...
def isPartOfSpeech(word,partOfSpeech):
    if(partOfSpeech=="D"):
        return (word in dets) or (word in pronouns) #these should probably be combined. ORRR maybe not. A pronoun can make up a DP and doesn't use a noun, but a det probably always uses a noun. Just allow some things to be both a det and a pro.
    if(partOfSpeech=="V"):
...
def tagPartsOfSpeech(word):
    partsOfSpeechForWord=[]
    for partType in ["D","V","N","P","AUX","A","Adv"]:
        if(isPartOfSpeech(word,partType)):
            partsOfSpeechForWord.append(partType)
    if(len(partsOfSpeechForWord)==0):
        partsOfSpeechForWord.append("U")
    return (word,partsOfSpeechForWord)

def isPartOfSpeech2(taggedWord,partOfSpeech):
    try:
        if(partOfSpeech in taggedWord[1] or ('U' in taggedWord[1] and partOfSpeech in ["D","V","N","P","AUX","A","Adv"])):
            return True
    except:
        pass
    return False

def getFullParsings(sentenceArr,maxDepth=50,partType='IP'):
    parsingResults=[parseResult[0] for parseResult in parsePartNode(partType,sentenceArr,maxDepth) if parseResult[1]==[]]#find all the ways to parse the sentence, that don't leave extra words at the end.
    return parsingResults

def grabUParts(parseTreeArr):
    UParts=[]
    if(type(parseTreeArr)!=list):
        return UParts
    try:
        if(type(parseTreeArr[0])==tuple and parseTreeArr[0][0]=="U"):
            return [parseTreeArr]
    except:
        print "err with",parseTreeArr
        return UParts
    for subtree in parseTreeArr:
        UParts+=grabUParts(subtree)
    return UParts

def newWordsTypes(maxDepth=50):
    sentenceString=raw_input("Enter sentence.").lower()
    sentenceArr=sentenceArrify(sentenceString)#split the sentence into a list of words
    sentenceArr=map(tagPartsOfSpeech,sentenceArr)
    return grabUParts(getFullParsings(sentenceArr,maxDepth))
...
class parsePartNode:
    ...
    def next(self):
        ...
        if(i==-1 and self.partType==None):
            ...
            return self.previousResult,self.tokensList[1:]
        elif(i==-1 and isPartOfSpeech2(self.tokensList[0],self.partType)):#incomplete
            self.i=0#incomplete
            if("U" in self.tokensList[0][1]):
                self.previousResult=[("U",self.partType),self.tokensList[0][0]]#,self.tokensList[1:] 
            else:
                self.previousResult=[self.partType,self.tokensList[0][0]]#,self.tokensList[1:]                
            if(verbose):print "found a",self.partType,". It is",self.tokensList[0]
            #print "returning thing word literal:",self.partType,self.tokensList[0],"and",self.tokensList[1:]
            return self.previousResult,self.tokensList[1:]
        else:
